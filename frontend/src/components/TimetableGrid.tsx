...
import { 
  Calendar, Layers, User, Users, DoorOpen, DoorClosed, Clock, AlertCircle, 
...
export function TimetableGrid({
  ...
  const selectedTeacher = teachers.find((t: any) => t.id === Number(selectedTeacherId)) || teachers[0];
  const selectedRoom = rooms.find((r: any) => r.id === Number(selectedRoomId)) || rooms[0];
  const selectedDay = days.find((d: any) => d.id === Number(selectedDayId)) || days[0];

  // Grid headers: Max periods in any day
  const maxPeriodCount = Math.max(...days.map((d: any) => (d.periods ? d.periods.length : 0)), 6);

  const handleSlotClick = async (period: any, currentEntry: any) => {
    ...
    if (!selectedEntry) {
      if (currentEntry) {
        ...
        setActionMessage({
          type: "success",
          text: `Selected "${currentEntry.subject_name}" on ${currentEntry.day_name}. Click another slot to move or swap.`,
        });
      }
      return;
    }
    ...
    try {
      if (!currentEntry) {
        ...
        setActionMessage({ type: "success", text: "✓ Class moved successfully!" });
      } else {
        // Occupied slot -> Swap
        await api.swapClassSlots({
          timetable_id: timetable.id,
          entry_a_id: selectedEntry.id,
          entry_b_id: currentEntry.id,
        });
      ...
      setActionMessage({ type: "error", text: `✗ Action Rejected: ${err.message}` });
    } finally {
  ...
  return (
    <div className="space-y-4 font-sans">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 rounded-2xl border border-zinc-200 bg-white p-3.5 shadow-xs dark:border-zinc-800 dark:bg-zinc-900">
        <div className="flex items-center gap-2">
          <div className="inline-flex rounded-xl bg-zinc-100 p-1 border border-zinc-200 dark:bg-zinc-800 dark:border-zinc-700">
            <button
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="flex items-center gap-2 rounded-xl bg-zinc-900 px-3.5 py-1.5 text-xs font-bold text-white shadow-xs hover:bg-zinc-800 dark:bg-zinc-100 dark:text-zinc-900 dark:hover:bg-zinc-200 active:scale-95 transition-all cursor-pointer"
          >
        ...
        <div className="flex items-center gap-2">
          {viewMode === "section" && (
            <select
              ...
              onChange={(e) => { setSelectedSectionId(Number(e.target.value)); setSelectedEntry(null); }}
              className="w-full md:w-auto rounded-xl border border-zinc-200 bg-zinc-50 px-3 py-1.5 text-xs font-bold text-zinc-900 dark:border-zinc-700 dark:bg-zinc-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-zinc-900"
            >
              {sections.map((sec: any) => (
                <option key={sec.id} value={sec.id}>
                  Section: {sec.name} ({sec.semester_name || "Semester"})
                </option>
              ))}
            </select>
          )}
          {viewMode === "teacher" && (
            <select
              ...
              onChange={(e) => { setSelectedTeacherId(Number(e.target.value)); setSelectedEntry(null); }}
              className="w-full md:w-auto rounded-xl border border-zinc-200 bg-zinc-50 px-3 py-1.5 text-xs font-bold text-zinc-900 dark:border-zinc-700 dark:bg-zinc-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-zinc-900"
            >
              {teachers.map((t: any) => (
          ...
          {viewMode === "room" && (
            <select
              ...
              onChange={(e) => { setSelectedRoomId(Number(e.target.value)); setSelectedEntry(null); }}
              className="w-full md:w-auto rounded-xl border border-zinc-200 bg-zinc-50 px-3 py-1.5 text-xs font-bold text-zinc-900 dark:border-zinc-700 dark:bg-zinc-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-zinc-900"
            >
              {rooms.map((r: any) => (
          ...
          <div className="flex items-center gap-1.5 border-l border-zinc-200 dark:border-zinc-800 pl-2">
            <button
              ...
              className="flex items-center gap-1 rounded-lg border border-zinc-200 bg-white px-2.5 py-1.5 text-xs font-semibold text-zinc-700 hover:bg-zinc-50 dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-200 shadow-xs cursor-pointer"
              title="Export as PDF"
            >
        ...
        <div
          className={cn(
            ...
            actionMessage.type === "success"
              ...
              : "bg-zinc-100 text-zinc-900 border border-zinc-300 dark:bg-zinc-800 dark:border-zinc-700 dark:text-zinc-100"
          )}
        >
          <div className="flex items-center gap-2">
            {actionMessage.type === "success" ? (
              <CheckCircle2 className="h-4 w-4 shrink-0 text-white dark:text-zinc-900" />
            ) : (
              <AlertCircle className="h-4 w-4 shrink-0 text-zinc-700 dark:text-zinc-300" />
            )}
                        ...
                        {Array.from({ length: maxPeriodCount }).map((_, pIdx) => {
                          ...
                          return (
                            ...
                            >
                              ...
                              {matchEntry ? (() => {
                                const teacherTheme = getTeacherColor(matchEntry.teacher_name || matchEntry.teacher_id);
                                return (
                                <div
                                  className={cn(
                                    "group relative flex flex-col justify-between rounded-xl p-2.5 text-xs transition-all shadow-2xs border border-l-4",
                                    teacherTheme.border,
                                    matchEntry.room_type_name?.includes("Lab")
                                      ? "bg-zinc-50/90 text-zinc-900 border-zinc-300 hover:border-zinc-500"
                                ...
                                >
                                  ...
                                  <div className="mt-1.5 space-y-1 text-[10px] text-zinc-600">
                                    {viewMode !== "teacher" && (
                                      <div className="flex items-center gap-1.5 font-medium truncate">
                                        <span className={cn("h-4 w-4 rounded-full flex items-center justify-center text-[8px] font-black shrink-0", teacherTheme.avatar)}>
                                          {(matchEntry.teacher_name || "T")[0]}
                                        </span>
                                        <span className="font-semibold text-zinc-900 truncate">{matchEntry.teacher_name}</span>
                                      </div>
                                    )}
                                    {viewMode !== "room" && (
